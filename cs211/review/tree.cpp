...
private:
 struct TreeNode
 {
  ...
  void displayInPreorder(TreeNode *)const ; 
  void displayInPostorder(TreeNode *) const ; 
...
public:
  ...
  void displayInPreorder() const {displayInPreorder(root);}
  void displayInPostorder() const {displayInPostorder(root);} 
...
void IntBinaryTree::displayInPreorder(TreeNode *nodePtr) const
{
  if(nodePtr) 
    {
      cout << nodePtr-> value << " " ; 
      displayInPreorder(nodePtr-> left);
      displayInPreorder(nodePtr->right) ; 
    }
}

void IntBinaryTree::displayInPostorder(TreeNode *nodePtr) const
{
  if(nodePtr)
    {
      displayInPostorder(nodePtr->left) ; 
      displayInPostorder(nodePtr-> right) ; 
      cout << nodePtr-> value << " " ; 
    }
}
//*********************************************************
...
// by visits the root nodes first (pre-order traversal)
  ...
  if(nPtr)
    {
      ...
      newRoot-> value = nPtr-> value ; 
      newRoot-> left = copyTree(nPtr->left) ; 
      newRoot-> right = copyTree(nPtr-> right) ; 
    }
  else 
    return NULL ;
...
int IntBinaryTree::getTreeHeight(TreeNode* nodePtr)
{
  if(!nodePtr)
    ...
    {
      int  l = getTreeHeight(nodePtr->left) ; 
      int   r = getTreeHeight(nodePtr-> right) ; 
      if(l > r)
	return 1 + l ; 
      else 
	return 1 + r ; 
...
int main()
{
  ...
  cout << "Here are the values in the tree in preorder:\n";
  tree.displayInPreorder();
  cout << endl ; 

  cout << "Here are the values in the tree in postorder:\n";
