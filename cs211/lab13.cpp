...
if(nPtr)
    ...
    new_root->value=nPtr->value;
    new_root->left=copyTree(nPtr->left);
    new_root->right=copyTree(nPtr->right);
      } 
else return NULL;
...
int IntBinaryTree::countNodes(TreeNode *&nodePtr)
{
  ...
  if ( nodePtr == NULL )
    ...
    return 1 + countNodes(nodePtr->left) +  countNodes(nodePtr->right);  // Add the number of nodes
    //     in the left subtree.
...
int IntBinaryTree::treeHeight()
{
  ...
  return  getTreeHeight(root); //pass root to start at root  
}
//*************************************************************
...
int IntBinaryTree::getTreeHeight(TreeNode* nodePtr)
{
  if(!nodePtr) 
    ...
    {
      int lh = getTreeHeight(nodePtr-> left);
      int rh = getTreeHeight(nodePtr-> right) ; 
      if(lh> rh)
	return  1 + lh ; 
      else 
	return 1 + rh ; 
...
int main()
{
  ...
  cout << endl;
    ...
    cout << "Num of nodes: " << tree2.numNodes() << endl  ;
