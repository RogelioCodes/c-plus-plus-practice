...
bool syntaxerror1(tokentype expected, string saved_lexeme);
bool syntaxerror2(string funcName, string saved_lexeme);
...
// i.e. Done by:Rogelio Cordova
bool syntaxerror2( string funcName, string saved_lexeme){
  cout << "SYNTAX ERROR: unexpected "<< saved_lexeme <<" found in "<< funcName << endl;
  exit(1);//quit program
}
...
bool match(tokentype expected ){
  if(next_token() != expected) // if the next token does not match
    { syntaxerror1(expected, saved_lexeme); 
      return false;
    } //generates syntax error message by calling the function "syntaxerror1" 
  else{
    if (DEBUG)  cout<<"Matched "<<tokenName[expected]<<endl;
    token_available = false; // sets token to not available
    return true;
  }
}
