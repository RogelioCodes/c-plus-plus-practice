...
void llist::displayAll()
{
  ...
  if (isEmpty())                 // special case. checks if empty.
    cout << "[ empty ]" << endl; // displays: [ empty ] if empty 
...
if (isEmpty())
  throw Underflow(); //Error case: underflow is thrown when the list is empty
    ...
    OldNum = Front->Elem; // oldNum is set to the Front->element, the node that is returned when there is more than one element
...
// If there is more than one node the rear node elemt is given back through OldNum and the rear node is also removed.
