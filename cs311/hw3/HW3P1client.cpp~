...
void caseOne()
{
  ...
  cout << 1 <<  " Step - Check if empty, report results" << endl;
  if (L.isEmpty())
  ...
  //2 display the list
  cout << 2 << " Step - Display the list " << endl;
  L.displayAll();

  //3 add 4 integers 1,2,3,4
  cout << 3 << " Step - Add 4 integers" << endl;
  L.addRear(1);
  L.addRear(2);
  L.addRear(3);
  L.addRear(4);
  ...
  //4 display the list
  cout << 4 <<  " Step - Display list " << endl;
  L.displayAll();

  //5 remove from front twice (and display the elements removed)
  cout << 5 << " Step - Remove from front twice" << endl;
  L.deleteFront(x);
  cout << "Delete: " << x << " ";
  L.deleteFront(x);
  cout << x << endl;
  ...
  //6 display the list
  cout << 6 << " Step - Display list" << endl;
  L.displayAll();

  //7 check empty and report the result
  cout << 7 << " Step - Check if empty again and report result" << endl;
  if(L.isEmpty())
    cout << "List is Empty" << endl;
  else
    cout << "List is not Empty" << endl;
  ...
  cout << 8 << " Step - Remove from the rear twice (display elements removed)" << endl;
  L.deleteRear(x);
  cout << "Removed: " << x << " ";
  L.deleteRear(x);
  cout << x << endl;
  ...
  cout << 9 << " Step - Check if empty again and report result" << endl;
  if (L.isEmpty())
    cout << "List is empty" << endl;
  else
    cout << "List is not empty" << endl;
...
void caseTwo()
{
  ...
  cout << c << " Step - Add front (5)" << endl; c++;
  L2.addFront(5);

  // 2.add to front again  (element 4)
  cout << c << " Step - Add front (4)" << endl; c++;
  L2.addFront(4);

  // 3.delete Front
  cout << c << " Step - Delete front" << endl; c++;
  L2.deleteFront(x);

  // 4.add to rear 3 times (elements 6,8,9)
  cout << c << " Step - Add to rear 3 times (6,8,9) " << endl; c++;
  L2.addRear(6);
  L2.addRear(8);
  L2.addRear(9);

  // 5. display all
  cout << c << " Step - Display list" << endl; c++;
  L2. displayAll();

  // 6.add before the 1st  (element 4) . 4 5 6 8 9
  cout << c <<  " Step - Add before 1st element (4)" << endl; c++;
  L2.addbeforeIth(1,4);

  // 7.add before the 4th  (element 7) . 4 5 6 7 8 9
  cout << c <<  " Step - Add before the 4th element (7)" << endl; c++;
  L2.addbeforeIth(4, 7);
  //delete this------------------------------------------------------------------------------
  //L2.displayAll();
  // 8.add before the 7th  (element 10) . 4 5 6 7 8 9 10
  cout << c << " Step - Add before the 7th element(10)" << endl; c++;
  L2.addbeforeIth(7, 10);

  // 9.add before the 9th  (element 12) . error (out of range)
  cout << c << " Step - Add before the 9th (element 12)" << endl; c++;
  ...
  cout << c << " Step - Add before the 0th (element 0)" << endl; c++;
  ...
  cout << c <<  " Step - Delete from rear until it is empty " << endl; c++;
  while(!L2.isEmpty()) {
    L2.deleteRear(x);
    cout << "Removed: " << x << endl;
  }
...
void caseThree()
{
  ...
  cout << 1 <<  " Step - Add before 0th element" << endl;;
  try {L3.deleteIth(0,x);}
  catch (llist::OutOfRange){cout << "Error (Out of range) " << endl;}

  //2.delete front . error (underflow)
  cout  << 2 << " Step - Delete front " << endl;
...
void caseFour()
{
  ...
  cout << 1 << " Step - Delete 2nd elem" << endl;
  try {L4.deleteIth(2,x);}
  catch (llist::OutOfRange){cout << "Error (Out of range)" << endl;}
  //    2.delete rear . error (underflow)
  cout << 2 << " Step - Delete rear" << endl;
  try {L4.deleteRear(x);}
  catch (llist::Underflow){cout << "Error (Underflow)" << endl;}
} // end of case 4
